...
const observable = getSymbolObservable(typeof window === 'undefined' ? {} : window);
...
class Subscription {
  ...
  unsubscribe() {
    ...
    let index = -1;
    let len = _parents ? _parents.length : 0; // if this._parent is null, then so is this._parents, and we
...
const rxSubscriber = typeof Symbol === 'function' && typeof Symbol.for === 'function' ? Symbol.for('rxSubscriber') : '@@rxSubscriber';
...
class Subscriber extends Subscription {
  ...
  constructor(destinationOrNext, error, complete) {
    ...
    _defineProperty$1(this, "syncErrorValue", null);

    _defineProperty$1(this, "syncErrorThrown", false);

    _defineProperty$1(this, "syncErrorThrowable", false);

    _defineProperty$1(this, "isStopped", false);

    _defineProperty$1(this, "_parentSubscription", null);
...
class SafeSubscriber extends Subscriber {
  constructor(_parentSubscriber, observerOrNext, error, complete) {
    ...
    this._context = context; // @ts-ignore
...
class SubjectSubscription extends Subscription {
  ...
  unsubscribe() {
    ...
    this.closed = true;
    const subject = this.subject; // @ts-ignore: Object is possibly 'null'
...
class Subject extends Observable {
  ...
  constructor() {
    ...
    _defineProperty$1(this, "observers", []);

    _defineProperty$1(this, "closed", false);

    _defineProperty$1(this, "isStopped", false);

    _defineProperty$1(this, "hasError", false);

    _defineProperty$1(this, "thrownError", null);
  }
  /**@nocollapse */
...
_defineProperty$1(Subject, "create", (destination, source) => {
  return new AnonymousSubject(destination, source);
});

class AnonymousSubject extends Subject {
  constructor(destination, source) {
    super(); // @ts-ignore: Object is possibly 'null'.
...
class DistinctUntilChangedSubscriber extends Subscriber {
  constructor(destination, compare, keySelector) {
    ...
    _defineProperty$1(this, "hasKey", false);
...
class BaseStoreService {
  ...
  select(mapFn) {
    ...
    return this.state$.pipe(map(state => mapFn(state)), distinctUntilChanged());
  }

  selectCommand(mapFn) {
    // return this.commandMapper$.asObservable().pipe(
    return this.commandMapper$.pipe(map(state => mapFn(state)));
  }
...
class AnyTypeStoreService extends BaseStoreService {
  constructor() {
    super(initialState);

    _defineProperty$1(this, "$state", this.select(state => state));
  ...
  sendCommand(command, argument, sender) {
    const commandObj = {
      sender,
      command,
      argument
...
let ScopedStoreComponent = Component(_class = class ScopedStoreComponent extends Vue {
  constructor(...args) {
    ...
    _defineProperty$1(this, "dataTranManager", scopedStoreManager);

    _defineProperty$1(this, "subscriptionsForPage", []);

    _defineProperty$1(this, "subscriptionsForGlobal", []);

    _defineProperty$1(this, "pathMapForPage", new Map());

    _defineProperty$1(this, "pathMapForGlobal", new Map());
  ...
  setupStoreProperty(args) {
    const opt = args.keyStore[args.key]; // const storeKey = (opt.path || key) as string;
  ...
  setPageDataCallback(callback, storePath) {
    ...
    if (this.dataTranManager.pageDataService) {
      ...
      let sub = this.dataTranManager.pageDataService.$state.subscribe(({
        payload,
        updater
      }) => {
        try {
  ...
  sendPageCommand(command, argument) {
  ...
  setPageCommandCallback(callback) {
    // this.initPageDataService();
    if (this.dataTranManager.pageDataService) {
      this.subscriptionsForPage.push(this.dataTranManager.pageDataService.$command.subscribe(command => {
        try {
  ...
  sendGlobalCommand(key, command, argument) {
  ...
  setGlobalCommandCallback(key, callback) {
    const service = this.dataTranManager.findOfCreateGlobalService(key);

    if (service) {
      this.subscriptionsForGlobal.push(service.$command.subscribe(command => {
        try {
          callback(command.command, command.argument);
        } catch (e) {
...
function sendGlobalData(data, storePath, sendOpt) {
  const vm = this;
